...
3. The most time consuming operation was the computation of transform matrices. To speed up the inverse kinematics calculations, these transform matrices were computed only once, at the start of IK_server.py, using a python class RobotTransform and the results were then used by encapsulating the required functions. This greatly sped up the process of calculating inverse kinematics, as compared to the initial approach of performing calculations after each request to IK solver service.
4. The only anomoly was observed during the calculation of the inverse of the rotation matrix from origin to joint 3 frame i.e. R0_3. The suggested code as given below failed to work as expected.
```python
